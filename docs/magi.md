...
These meta-agents do not directly contribute to a single session's answer, but instead analyze patterns across many sessions, enhancing long-term reasoning and system introspection.
...
MAGI agents exist *above* the quorum process. They analyze session records and R2 ledger entries, track trends, and suggest or enact changes over time.
