...
- Runs the full analysis pipeline on every request:
  1. **Deliberation** (default on) — after initial response collection, each agent reads its peers' responses and produces a refined reply; deliberated responses feed all downstream stages
  2. Dissent analysis (internal agreement between agents)
  3. NCG headless baseline and drift detection
  4. Semantic quorum aggregation (66% similarity threshold)
  5. R2 scoring, signal detection, and ledger indexing
  6. Session persistence
- Returns structured JSON with agent responses, deliberation summary, consensus, dissent metrics, NCG benchmark, and R2 grade
...
### Deliberation Engine (`maestro/deliberation.py`)
- **Cross-agent response sharing** — after initial parallel collection, each agent receives a structured prompt containing the original question, its own previous answer, and every peer agent's answer (identified by model/vendor name)
- **Configurable rounds** — default 1 round; set `deliberation_rounds` (1–5) for deeper debate. Each round adds one API call per participating agent
- **Default on** — controlled by `deliberation_enabled` (default `True`); disable via the API request or by passing `deliberation_enabled=False` to any orchestration entry point
- **Non-fatal** — errors during deliberation leave the agent's previous-round response intact; the pipeline always continues
- **Full history** — `DeliberationReport` records every round (`DeliberationRound`) with per-agent responses; the `final_responses` dict replaces initial responses for all downstream stages
- **Deliberated positions feed analysis** — dissent analysis, NCG drift detection, and semantic quorum all operate on post-deliberation outputs, not initial responses
- See [`deliberation.md`](./deliberation.md) for full documentation
   ...
   Orchestrator Foundry      direct call  direct  HTTP/SSE
              ...
              +----------+----------+-----+--------+
               ...
               maestro.orchestrator (core pipeline)
                         |
                         v
   Conversational Track (parallel, all agents simultaneously)
   [GPT-4o, Claude Sonnet 4.6, Gemini 2.5 Flash, Llama 3.3 70B, ShardNet]
                         |
                         v
   Deliberation Engine (default ON, 1 round)
   Each agent reads peer responses → produces refined reply
   [GPT-4o sees Claude + Gemini + Llama → refines]
   [Claude Sonnet sees GPT-4o + Gemini + Llama → refines]
   [...] (configurable rounds; deliberated responses replace initial)
                         |
         +---------------+---------------+
         |               |               |
   Dissent Analysis  NCG Track     (deliberated responses
   (pairwise         [Headless      used for all tracks)
    distance,         Gen]
    outlier det.)      |
         |             v
         |       Drift Detector
