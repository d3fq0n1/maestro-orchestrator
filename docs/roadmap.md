...
- **Progress bar fix, deliberation feedback, Prompt screen polish (v7.4.0)** -- Boot progress bar replaced asymptotic curve (stuck at 1-3%) with linear ramp to 80% giving accurate visual feedback. TUI now handles `deliberation_start`, `deliberation_round`, and `deliberation_done` SSE events with agent status cycling, round-by-round stage updates, and response viewer messages — deliberation is no longer invisible. Prompt screen gains a live Pipeline Preview panel showing agent roster, deliberation config, and analysis stages. Preview updates reactively when deliberation toggle/rounds change.
